...
# Suppress warning messages to keep the console output clean.
...
# Extracts the top N most important features from a fitted CatBoost model.
def get_top_features(fitted_model, feature_names, top_n=10):
    """
    Returns the top N features ranked by CatBoost's built-in feature importance.
    """
...
def run_shap_summary(model, X_test, dataset_name, phase_label):
    """
    Generates and saves a SHAP summary plot for a trained CatBoost model.
    ...
    TreeExplainer is used because it is designed for tree based models such as CatBoost.
    """
...
PARAM_GRID = {
    'iterations'   : [200, 500],       # Number of boosting rounds / trees.
...
# Runs the full modelling pipeline for all dataset versions.
def run_pipeline():
    ...
    print("=" * 60)
    print("PHASE 1: CATBOOST + GRID SEARCH — Club Identity Excluded")
    print("=" * 60)

    # Train and evaluate a CatBoost model for each dataset version.
    for name, (train_file, test_file) in datasets.items():
        ...
        # Set up CatBoost with GridSearchCV to tune hyperparameters using 3 fold cross validation.
        ...
        # Extract the best trained CatBoost model and its selected hyperparameters.
